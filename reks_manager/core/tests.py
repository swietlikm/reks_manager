...
from .models import Allergy, ALLERGY_CATEGORY
...
class AllergyModelTest(TestCase):
    ...
    def test_allergy_creation(self):
        ...
        self.assertEqual(str(self.allergy_instance), _("Allergy") + f": {self.allergy_data['category']} {self.allergy_data['name']}")
...
class AllergyViewTest(TestCase):
    ...
    def test_create_allergy_view(self):
        ...
        response = self.client.post(url, data={'category': ALLERGY_CATEGORY[0][0], 'name': 'NewName', 'description': 'NewDescription'})
    ...
    def test_delete_allergy_view(self):
        ...
        with self.assertRaises(Allergy.DoesNotExist):
            deleted_allergy = Allergy.objects.get(pk=self.allergy.pk)
